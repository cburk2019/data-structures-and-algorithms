...
// ^ and $ checks from the beginning to the end of given string, number set, etc
// no need for a \ as we wrapped what we were testing in ()

// does not work, first try /^\d{5,10}$/.test(word);
// does not work, second try /^\[5-10]$/.test(word);
// };
...
// lookahead assertion
// does not work, second try /^(?=1)/.test(string);
// does not work, first try /\b[0-9]\d[a-zA-Z]/.test(string);
// };
...
const validateEmail = (email) => /h/.test(email);
...
// does not work, first try /^\(?\d{3}\)?[\s.-]\d{3}[\s.-]\d{4}$/.test(phoneNumber);
// };
