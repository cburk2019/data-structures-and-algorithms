...
const raisedToTheThird = (arr) => {
  const newArray = [];
  arr.forEach((arrItem) => {
    const newNum = Math.pow(arrItem, 3);
...
const addOne = (arr) => {
  // const newNum = arr.map((newNum) => newNum + 1);
  // return newNum;
  return arr.map((newNum) => newNum + 1);
};
...
const addQuestion = (arr) => {
  return arr.map((arrayItem) => arrayItem + "?");
};
...
const forEachTwoToThe = (arr) => {
  ...
  arr.forEach((arrItem) => {
    const newNum = Math.pow(2, arrItem);
    newArray.push(newNum);
  });
  return newArray;
};
