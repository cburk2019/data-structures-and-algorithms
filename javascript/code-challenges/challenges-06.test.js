...
const getNames = (arr) => {
  // arr.map()
  // return arr.reduceRight(arr.name);
  // arr.map((_, idx, arr) => arr[arr.length - 1 - idx]);
  // arr.slice(0).reverse().map(arr.name);
  // return arr.split(`${arr.name}`).reverse().join(`${arr.name}`);
  return arr.map((item) => {
...
const addBirthYearProperty = (obj, year) => {
  // let newObject = {};
  // newObject.assign(obj, year);
  // let merged = { ...obj, ...year};
  // obj.merge(year);
  // Object.assign(obj, year);
  obj.yearBorn = year;
};
