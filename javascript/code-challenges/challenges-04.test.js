...
const updateAnimal = (arr, callback) => {
  const addAnimal = [];
  arr.forEach((arrItem) => {
    addAnimal.push(callback(arrItem));
  });
  return addAnimal;
};

// callback - declaring function w/2 parameters (arr, callback)
...
// also this works >>> const sortNames = (arr) => arr.sort();
